...
if(sil){
	...
	for(let el of Object.values(vmd["@graph"])){
		if("VideoObject" == el["@type"]){
			console.log(el);
			let videoContainer = document.getElementsByClassName("rc-VideoPlayerContainer__video-wrapper")[0],
			titleEl = document.getElementsByClassName("rc-VideoPlayerContainer__title")[0];

			let vt = document.createElement("VIDEO");
			vt.src = el["contentURL"];
			vt.poster = el["thumbnailURL"];
			vt.controls = !0;
			videoContainer.parentElement.replaceChild(vt, videoContainer);

			let l = document.getElementsByClassName("other-video-link")[0];
			l.href = el["contentURL"];
			l.download = el["name"] + ".mp4";

			let a = document.createElement("A");
			a.textContent = titleEl.textContent;
			a.href = l.href;
			a.download = l.download;
			titleEl.textContent = "";
